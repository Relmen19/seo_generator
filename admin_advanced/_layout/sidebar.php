...
<aside class="shrink-0 flex flex-col items-center gap-6 py-6 px-2 md:px-3">
  ...
  <nav class="bg-sand-50 rounded-full shadow-rail py-3 px-2 flex flex-col gap-2"
       x-data="{ hasProfile: !!localStorage.getItem('seo_profile_id') }"
       x-init="window.addEventListener('storage', e => { if (e.key === 'seo_profile_id') hasProfile = !!e.newValue });
               window.addEventListener('seo-profile-changed', e => { hasProfile = !!localStorage.getItem('seo_profile_id') })">
    <?php foreach ($nav as $item): $isActive = ($item['id'] === $activeNav); ?>
      <?php if ($item['gated']): ?>
        ...
        <span x-show="!hasProfile"
              title="<?= htmlspecialchars($item['label'], ENT_QUOTES, 'UTF-8') ?> — выберите профиль"
              class="w-11 h-11 rounded-full grid place-items-center text-ink-300 cursor-not-allowed opacity-40">
          <svg viewBox="0 0 20 20" width="20" height="20" fill="none" stroke="currentColor"><?= $item['icon'] ?></svg>
        </span>
      <?php else: ?>
        <a href="<?= htmlspecialchars($item['href'], ENT_QUOTES, 'UTF-8') ?>"
