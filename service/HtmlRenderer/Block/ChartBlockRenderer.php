...
class ChartBlockRenderer extends AbstractBlockRenderer
{
        ...
        if ($useCSS && !empty($labels)) {
            ...
            if (count($data) <= 8 && count($data) > 0) {
                ...
                $h .= '<circle cx="100" cy="100" r="' . $radius . '" fill="none"'
                    . ' stroke="var(--color-border)" stroke-width="' . $strokeW . '" />';
    ...
    {
        return '.block-chart { }'
            . "\n" . '.chart-desc { font-size:14px; color:var(--color-text-3); line-height:1.6; margin-bottom:20px; max-width:560px }'
            ...
            . "\n" . '.chart-label { color:var(--color-text-2); font-weight:500; font-size:13px; text-align:right; white-space:normal; line-height:1.3 }'
            ...
            . "\n" . '.chart-val { font-family:var(--type-font-heading); font-weight:700; color:var(--color-text); font-size:14px }'
            . "\n" . '.donut-layout { display:grid; grid-template-columns:auto 1fr; grid-template-rows:auto 1fr; gap:8px 36px; align-items:start }'
            . "\n" . '.donut-aside-desc { grid-column:2; grid-row:1; font-size:14px; color:var(--color-text-3); line-height:1.65; margin:0; padding-top:4px }'
            ...
            . "\n" . '.donut-svg-wrap .donut-hole { position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); width:56%; height:56%; border-radius:50%; background:var(--color-surface); display:flex; flex-direction:column; align-items:center; justify-content:center; box-shadow:inset 0 2px 12px rgba(0,0,0,.04); pointer-events:none }'
            . "\n" . '[data-theme="dark"] .donut-svg-wrap .donut-hole { background:var(--color-surface) }'
            . "\n" . '.donut-total { font-family:var(--type-font-heading); font-size:1.8rem; font-weight:900; color:var(--color-text); letter-spacing:-1px; line-height:1 }'
            . "\n" . '.donut-total-label { font-size:.72rem; color:var(--color-text-3); text-transform:uppercase; letter-spacing:1.5px; margin-top:4px }'
            . "\n" . '.donut-legend { display:flex; flex-direction:column; gap:6px }'
            . "\n" . '.donut-legend-item { display:flex; align-items:center; gap:8px; padding:8px 14px; border-radius:10px; background:rgba(255,255,255,.5); border:1px solid var(--color-border); transition:all .25s; min-width:0; cursor:pointer; font-family:var(--type-font-text); font-size:inherit; color:inherit }'
            ...
            . "\n" . '.donut-legend-item.is-active { border-color:var(--color-accent); box-shadow:0 2px 12px rgba(37,99,235,.12); background:var(--color-accent-soft) }'
            ...
            . "\n" . '.donut-legend-text { font-size:13px; color:var(--color-text-2); font-weight:500; white-space:normal; min-width:0; overflow:hidden }'
            . "\n" . '.donut-legend-val { font-family:var(--type-font-heading); font-size:13px; font-weight:700; color:var(--color-text); margin-left:auto; white-space:nowrap }'
            . "\n" . '.donut-legend-val small { font-weight:400; color:var(--color-text-3); font-size:11px }'
            ...
            . "\n" . '.donut-detail-wrap { padding:14px 16px; border-radius:12px; background:rgba(255,255,255,.45); border:1px solid var(--color-border); backdrop-filter:blur(6px) }'
            ...
            . "\n" . '.donut-detail-label { font-family:var(--type-font-heading); font-size:14px; font-weight:700; color:var(--color-text) }'
            . "\n" . '.donut-detail-desc { font-size:13px; color:var(--color-text-3); line-height:1.55 }'
