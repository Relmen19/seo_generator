...
class CtaBlockRenderer extends AbstractBlockRenderer
{
    ...
    public function getCss(): string
    {
        return '.block-cta { background:linear-gradient(135deg,rgba(37,99,235,.88) 0%,rgba(13,148,136,.88) 100%); backdrop-filter:blur(24px); color:#fff; text-align:center; position:relative; overflow:hidden; padding:100px 0 }'
            ...
            . "\n" . '.block-cta--transparent .btn-primary { background:var(--color-accent,#2563eb); color:#fff }'
            ...
            . "\n" . '.btn-primary { display:inline-block; background:#fff; color:var(--color-accent); font-family:var(--type-font-text); font-weight:700; font-size:1rem; padding:18px 40px; border-radius:100px; transition:all .25s; text-decoration:none }'
            . "\n" . '.btn-primary:hover { transform:translateY(-2px); box-shadow:0 10px 28px rgba(0,0,0,.2); text-decoration:none }'
            . "\n" . '.btn-outline { display:inline-block; background:transparent; color:#fff; font-family:var(--type-font-text); font-weight:600; font-size:1rem; border:2px solid rgba(255,255,255,.65); padding:16px 38px; border-radius:100px; transition:all .25s; text-decoration:none }'
