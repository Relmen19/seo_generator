...
class GaugeChartBlockRenderer extends AbstractBlockRenderer
{
    ...
    public function getCss(): string
    {
        return '@keyframes pulseRing{0%,100%{box-shadow:0 0 0 0 var(--pulse-c,rgba(37,99,235,.3))}50%{box-shadow:0 0 0 8px transparent}}'
            ...
            . "\n" . '.gauge-card{padding:24px 18px 18px;border-radius:18px;background:rgba(255,255,255,.55);backdrop-filter:blur(12px);border:1px solid var(--color-border);text-align:center;transition:all .35s;cursor:pointer}'
            ...
            . "\n" . '.gauge-card.is-active{border-color:var(--gauge-c,var(--color-accent));box-shadow:0 0 0 2px var(--gauge-c,var(--color-accent)),0 12px 40px rgba(37,99,235,.12)}'
            ...
            . "\n" . '.gauge-track{fill:none;stroke:var(--color-border);stroke-width:10;stroke-linecap:round}'
            ...
            . "\n" . '.gauge-val{font-family:var(--type-font-heading);font-size:1.5rem;font-weight:900;color:var(--color-text);letter-spacing:-1px;line-height:1}'
            . "\n" . '.gauge-val small{font-size:.5em;font-weight:500;color:var(--color-text-3);margin-left:2px}'
            . "\n" . '.gauge-name{font-size:12.5px;color:var(--color-text-3);margin-top:3px;font-weight:500}'
            . "\n" . '.gauge-range{display:flex;justify-content:space-between;font-size:9.5px;color:var(--color-text-3);margin-top:6px;padding:0 8px;font-family:var(--type-font-heading);font-weight:600;opacity:.4}'
            ...
            . "\n" . '.gauge-detail-inner{padding:16px 18px;border-radius:14px;background:rgba(255,255,255,.45);border:1px solid var(--color-border);backdrop-filter:blur(6px)}'
            ...
            . "\n" . '.gauge-detail-label{font-family:var(--type-font-heading);font-size:14px;font-weight:700;color:var(--color-text)}'
            . "\n" . '.gauge-detail-val{font-family:var(--type-font-heading);font-size:24px;font-weight:900;color:var(--color-text);margin-left:auto}'
            . "\n" . '.gauge-detail-desc{font-size:13px;color:var(--color-text-3);line-height:1.55}'
            . "\n" . '.gauge-detail-bar-track{height:6px;border-radius:100px;background:var(--color-border);overflow:hidden;margin-top:10px}'
