...
class HeroBlockRenderer extends AbstractBlockRenderer
{
    ...
    public function getCss(): string
    {
        return '.block-hero { position:relative; overflow:hidden; text-align:left }'
            ...
            . "\n" . '.hero-subtitle { font-size:clamp(1rem,2vw,1.25rem); font-weight:300; color:var(--color-text-2); max-width:720px; line-height:1.65; margin-bottom:1.5em }'
            . "\n" . '.hero-cta { display:inline-block; background:var(--color-accent); color:#fff; font-family:var(--type-font-text); font-size:1rem; font-weight:600; padding:18px 40px; border-radius:100px; transition:all .25s; box-shadow:0 8px 28px rgba(37,99,235,.35) }'
            . "\n" . '.hero-cta:hover { background:var(--color-accent); transform:translateY(-2px); box-shadow:0 12px 36px rgba(37,99,235,.45); text-decoration:none }'
