...
class MiniCalculatorBlockRenderer extends AbstractBlockRenderer
{
    ...
    public function getCss(): string
    {
        return '.mc-wrap{max-width:520px;margin:0 auto}'
            ...
            . "\n" . '.mc-label{font-family:var(--type-font-heading);font-size:13px;font-weight:600;color:var(--color-text)}'
            . "\n" . '.mc-select,.mc-input{padding:12px 16px;border:2px solid var(--color-border);border-radius:10px;font-size:15px;color:var(--color-text);background:var(--color-surface);font-family:var(--type-font-text);transition:border-color .2s;outline:none;width:100%;box-sizing:border-box}'
            . "\n" . '[data-theme="dark"] .mc-select,[data-theme="dark"] .mc-input{background:rgba(255,255,255,.05);color:var(--color-text)}'
            . "\n" . '.mc-select:focus,.mc-input:focus{border-color:var(--color-accent)}'
            . "\n" . '.mc-input-wrap{position:relative}'
            . "\n" . '.mc-unit{position:absolute;right:14px;top:50%;transform:translateY(-50%);font-size:13px;color:var(--color-text-3);font-weight:500}'
            . "\n" . '.mc-btn{width:100%;padding:14px;border:none;border-radius:12px;background:var(--color-accent);color:#fff;font-family:var(--type-font-heading);font-size:15px;font-weight:700;cursor:pointer;transition:all .2s}'
            . "\n" . '.mc-btn:hover{background:var(--color-accent);transform:translateY(-1px);box-shadow:0 4px 16px rgba(37,99,235,.25)}'
            . "\n" . '.mc-result{padding:20px;border-radius:16px;background:var(--color-accent-soft);text-align:center;margin-top:18px;animation:fadeInUp .5s ease}'
            . "\n" . '.mc-result-value{font-family:var(--type-font-heading);font-size:2rem;font-weight:900;color:var(--color-accent)}'
            . "\n" . '.mc-result-text{font-size:14px;color:var(--color-text-2);margin-top:4px}'
            . "\n" . '.mc-formula-desc{font-size:11.5px;color:var(--color-text-3);text-align:center;margin-top:12px}'
            . "\n" . '.mc-disclaimer{font-size:11.5px;color:var(--color-text-3);text-align:center;margin-top:8px;font-style:italic}'
            ;
    }
