...
class RangeComparisonBlockRenderer extends AbstractBlockRenderer
{
    ...
    public function getCss(): string
    {
        return '.rc-wrap{display:flex;flex-direction:column;gap:8px}'
            . "\n" . '.rc-row{display:grid;grid-template-columns:100px 1fr;gap:14px;align-items:center;padding:12px 16px;border-radius:12px;background:rgba(255,255,255,.4);border:1px solid var(--color-border);cursor:pointer;transition:all .3s}'
            . "\n" . '[data-theme="dark"] .rc-row{background:rgba(255,255,255,.02)}'
            . "\n" . '.rc-row:hover{border-color:rgba(37,99,235,.2)}'
            . "\n" . '.rc-row.is-active{border-color:var(--color-accent);box-shadow:0 0 0 2px var(--color-accent),0 4px 20px rgba(37,99,235,.08)}'
            . "\n" . '[data-ranges].has-active .rc-row:not(.is-active){opacity:.35}'
            . "\n" . '.rc-label{font-family:var(--type-font-heading);font-size:13px;font-weight:700;color:var(--color-text)}'
            . "\n" . '.rc-bars{display:flex;flex-direction:column;gap:6px}'
            . "\n" . '.rc-bar-row{display:flex;align-items:center;gap:8px}'
            . "\n" . '.rc-bar-name{font-size:10px;font-family:var(--type-font-heading);font-weight:600;color:var(--color-text-3);width:30px;text-align:right;flex-shrink:0}'
            . "\n" . '.rc-bar-track{flex:1;height:12px;border-radius:100px;background:var(--color-border);position:relative;overflow:hidden}'
            . "\n" . '.rc-bar-range{position:absolute;top:0;bottom:0;border-radius:100px;transition:left .8s ease,width .8s ease}'
            . "\n" . '.rc-bar-marker{position:absolute;top:-3px;bottom:-3px;width:3px;border-radius:2px;background:var(--color-text);transition:left .8s ease;z-index:2}'
            . "\n" . '.rc-bar-val{font-size:10px;font-family:var(--type-font-heading);font-weight:700;color:var(--color-text);width:38px;flex-shrink:0}'
            . "\n" . '.rc-toggle{display:flex;gap:4px;margin-bottom:14px}'
            . "\n" . '.rc-toggle-btn{padding:8px 16px;border:1px solid var(--color-border);background:transparent;border-radius:10px;font-family:var(--type-font-heading);font-size:12px;font-weight:600;color:var(--color-text-3);cursor:pointer;transition:all .25s}'
            . "\n" . '.rc-toggle-btn:hover{color:var(--color-text);background:rgba(37,99,235,.06)}'
            . "\n" . '.rc-toggle-btn.is-active{color:#fff;background:var(--color-accent);border-color:var(--color-accent)}'
            ...
            . "\n" . '.rc-detail-inner{padding:14px 18px;border-radius:12px;background:rgba(255,255,255,.45);border:1px solid var(--color-border);font-size:13px;color:var(--color-text-3);line-height:1.55}'
    ...
    public function getJs(): string
    {
        return '(function(){'
            ...
            . 'var status=val>=lo&&val<=hi?"<span style=\\"color:var(--color-success)\\">✓ В норме</span>":"<span style=\\"color:var(--color-danger)\\">⚠ Отклонение</span>";'
