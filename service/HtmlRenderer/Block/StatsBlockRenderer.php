...
class StatsBlockRenderer extends AbstractBlockRenderer
{
    ...
    public function getCss(): string
    {
        return '.block-stats { padding:60px 0 }'
            ...
            . "\n" . '.stat-card { padding:28px 16px; border-radius:var(--radius-md); background:var(--color-surface); border:1px solid var(--color-border); transition:all .25s }'
            . "\n" . '.stat-card:hover { box-shadow:0 8px 28px rgba(0,0,0,.06); transform:translateY(-4px); border-color:var(--color-accent) }'
            . "\n" . '.stat-value { font-family:var(--type-font-heading); font-size:2.2rem; font-weight:900; color:var(--color-accent); letter-spacing:-1px; line-height:1 }'
            . "\n" . '.stat-label { font-size:.85rem; color:var(--color-text-3); margin-top:8px }'
