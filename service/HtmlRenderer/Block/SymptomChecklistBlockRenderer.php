...
class SymptomChecklistBlockRenderer extends AbstractBlockRenderer
{
    ...
    public function renderHtml(array $c, string $id): string
    {
        ...
        $h = '<section id="' . $id . '" class="block-symcheck reveal' . ($bgStyle ? ' has-bg-img' : '') . '"' . $bgAttr . ' data-toc="' . $title . '">'
            ...
            . '<div class="bar-track"><div class="sc-bar bar-fill" style="background:var(--color-accent);width:0"></div></div></div>'
            . '<div class="sc-items">';
        $groups = [];
    ...
    public function getCss(): string
    {
        return '.sc-wrap{max-width:560px;margin:0 auto}'
            ...
            . "\n" . '.sc-progress-text{font-size:13px;color:var(--color-text-3);font-weight:600;margin-bottom:6px}'
            . "\n" . '.sc-count{color:var(--color-accent);font-family:var(--type-font-heading);font-weight:900}'
            . "\n" . '.sc-bar{transition:width .5s ease}'
            . "\n" . '.sc-group{margin-bottom:16px}'
            . "\n" . '.sc-group-label{font-family:var(--type-font-heading);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:var(--color-text-3);margin-bottom:8px}'
            ...
            . "\n" . '.sc-item:hover{background:var(--color-accent-soft);border-color:rgba(37,99,235,.12)}'
            . "\n" . '.sc-check{display:none}'
            . "\n" . '.sc-checkmark{width:22px;height:22px;border-radius:7px;border:2px solid var(--color-border);flex-shrink:0;display:flex;align-items:center;justify-content:center;transition:all .25s;font-size:12px;color:transparent}'
            . "\n" . '.sc-check:checked~.sc-checkmark{background:var(--color-accent);border-color:var(--color-accent);color:#fff}'
            . "\n" . '.sc-check:checked~.sc-checkmark::after{content:"✓"}'
            . "\n" . '.sc-text{font-size:14px;color:var(--color-text-2);line-height:1.4}'
            . "\n" . '.sc-badge-important{background:var(--color-danger);color:#fff;font-size:10px;font-weight:700;padding:2px 7px;border-radius:100px}'
            ...
            . "\n" . '.sc-result-label{font-family:var(--type-font-heading);font-size:1.3rem;font-weight:900;margin-bottom:4px}'
            . "\n" . '.sc-result-text{font-size:14px;color:var(--color-text-2);line-height:1.6}'
    ...
    public function getJs(): string
    {
        return '(function(){'
            ...
            . 'if(th.length&&count>0){var t=th[0];rLabel.textContent=t.label||"";rLabel.style.color=t.color||"var(--color-text)";'
            . 'rText.textContent=t.text||"";rIcon.textContent=score>=7?"🚨":score>=3?"⚠️":"✅";'
            . 'result.style.display="block";result.style.borderColor=t.color||"var(--color-border)";'
