...
class TocComponent implements ComponentInterface
{
    ...
    public function getCss(): string
    {
        return '.toc { position:fixed; left:16px; top:50%; transform:translateY(-50%); z-index:100; opacity:.12; transition:opacity .4s ease; width:160px }'
            ...
            . "\n" . '.toc-inner { background:rgba(248,250,252,.88); backdrop-filter:blur(20px); border:1px solid var(--color-border); border-radius:12px; padding:14px 10px; box-shadow:0 4px 24px rgba(15,23,42,.06) }'
            . "\n" . '[data-theme="dark"] .toc-inner { background:rgba(5,13,26,.88) }'
            . "\n" . '.toc-label { font-family:var(--type-font-heading); font-size:9px; font-weight:700; text-transform:uppercase; letter-spacing:2px; color:var(--color-text-3); margin-bottom:10px; padding:0 8px }'
            . "\n" . '.toc-link { display:block; font-size:11.5px; color:var(--color-text-3); padding:5px 8px; border-radius:6px; text-decoration:none; transition:all .2s; border-left:2px solid transparent; margin-bottom:1px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis }'
            . "\n" . '.toc-link:hover { color:var(--color-text); background:var(--color-accent-soft); text-decoration:none }'
            . "\n" . '.toc-link.active { color:var(--color-accent); font-weight:600; border-left-color:var(--color-accent); background:var(--color-accent-soft) }'
